...
4. For the authorized rollback, first require any present `design_cycle` to be a nonnegative integer; otherwise stop before writing. Then write `spec.md`: `status: <target>`, `updated: <today>`. When the source is `done`, increment `design_cycle` (missing legacy value means `0`); otherwise retain `design_cycle` unchanged. Append to `## Clarifications`: `**Revised (YYYY-MM-DD):** Rolled back from \`<current>\` to \`<target>\`. Change: <description>. Note: artefacts from phases after \`<target>\` (if any) are stale.`
...
- Only `spec.md` is written by the rollback owner: status, updated, `design_cycle` and revision note. `design_cycle` is a nonnegative integer, starts at `0`, and changes only on a `done` rollback. Delegated author, clarify, plan and review retain their separate artifact ownership.
