...
ADRs are proposed during `/maxi:plan` and `/maxi:implement`, then written only after explicit user approval.
...
- ADRs are append-only; supersede a decision instead of editing its body.
