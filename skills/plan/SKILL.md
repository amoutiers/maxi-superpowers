...
8. **Stop at the plan-review handoff** — for a marked root, report the current `plan.md` revision that requires external review. Do not invoke `/maxi:tasks`; `/maxi:x-review` must independently create the matching approval first. An unmarked root has no review handoff and proceeds through its ordinary next-step report.
9. **Report** — for a marked root: *"Plan written to `docs/maxi/specs/NNNN-slug/plan.md` (status: `planned`). Next: an external `/maxi:x-review` of the current plan revision; after approval, display the remaining continuation and obtain its own consent before `/maxi:tasks`."* For an unmarked root: *"Plan written to `docs/maxi/specs/NNNN-slug/plan.md` (status: `planned`). Next: `/maxi:tasks`."*
...
5. For a marked root, return the spec status only to `planned`, then invoke exactly `bash skills/revise/replay-plan.sh --spec-dir <spec-dir> --changed plan.md --previous-revision <captured> --start-phase tasks`. Display all planner records and stop at `REVIEW_REQUIRED|plan.md|<current-plan-revision>` for a fresh external plan review. For an unmarked root, return only to `planned` without a continuation, planner, or review report.
