...
awk -v completed="$COMPLETED" -v selected="$MAP" '
  BEGIN {
    while ((getline id < completed) > 0) done[id] = 1
    while ((getline line < selected) > 0) { split(line, task, "|"); selected_ids[task[2]] = 1 }
  }
  ($0 ~ /^- \[[^]]*\] T/ || $0 ~ /^[[:space:]]+- \[[^]]*\] T[0-9]/) && $0 !~ /^- \[[ xX]\] T[0-9][0-9][0-9] .+/ { bad = 1 }
  /^- \[[ xX]\] T[0-9][0-9][0-9] .+/ {
    id = substr($0, 7, 4)
    if (seen[id]++) bad = 1
    if (selected_ids[id]) sub(/^- \[[ xX]\]/, done[id] ? "- [x]" : "- [ ]")
  }
  { print }
  END { if (bad) exit 2 }
