...
- **Copy template first.** When creating a new constitution, verify `templates/constitution-template.md` exists (Read tool) before copying; if missing, stop: *"Cannot proceed — `templates/constitution-template.md` is missing. Please reinstall the maxi plugin."* Then copy it into `docs/constitution.md` before writing anything. Use the template's section structure. After elicitation, set the YAML frontmatter: `version: 1.0.0`, `ratified: [today's ISO date]`, `last_amended: [today's ISO date]`. When amending, bump the version (MAJOR.MINOR.PATCH) and update `last_amended`.
