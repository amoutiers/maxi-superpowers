...
- `x-adr` is internal and is never invoked directly by the user. Every new ADR records its creating spec through a direct `spec` link or uses `spec: null`, plus its `design_cycle`. A detected change to an accepted ADR linked to the current active spec with a matching `design_cycle` triggers an agent-proposed active-spec amendment, including after a rollback from `done`: `x-adr` shows the full amended ADR and exact diff, then writes only after explicit approval. A `done` rollback increments the cycle, so missing, null, different-cycle, or specs at `done`, `parked`, or `cancelled`, use closed-spec supersession instead. Supersession replaces the old slug in the active spec's `related_adrs` with the accepted successor, keeping review inputs current. This cross-cutting route does not add a status or transition.
