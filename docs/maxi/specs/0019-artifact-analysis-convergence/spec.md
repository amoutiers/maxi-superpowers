...
2. **Given** a spec at `tasked` with current artifacts and no open blocking finding, **When** analysis completes successfully, **Then** the report records a passing result and the spec transitions to `analyzed`.
...
**Independent Test**: Run two analyses around one approved correction cycle and verify finding identities, state changes, delta reporting, and the stop behavior after a second failure.
...
- A constitution change invalidates mutable descendants that recorded the previous constitution revision; the workflow reports the dependency path before proposing replay.
...
- A non-blocking finding is accepted or deferred: the decision must be explicit and remains visible in later reports.
...
- The runtime cannot provide a fresh reviewer context: the report records `self-review` and does not claim independent validation.
...
- **FR-005**: Analysis MUST transition `tasked` to `analyzed` only when its result is passing and its input artifacts are current.
- **FR-006**: Implementation MUST validate the current analysis result and input freshness in addition to checking `status: analyzed`.
...
- **FR-009**: Every governed write that changes a mutable artifact's content or metadata MUST increment `revision` in the same operation.
- **FR-010**: Mutable derived artifacts MUST record every direct mutable input and the exact input revision in a canonical `derived_from` representation.
...
- **FR-016**: The revision mechanism MUST apply to mutable per-project Maxi artifacts, including the constitution, specs, plans, research, data models, contracts, tasks, and analyses.
...
- **FR-033**: A single deterministic validation contract MUST govern artifact revision schema, dependency existence, freshness, cycles, ADR references, status compatibility, and explicit requirement references.
...
- **FR-039**: Semantic analysis SHOULD run in a fresh reviewer context that did not author the artifacts under review.
- **FR-040**: Every analysis report MUST declare `review_mode` as either `independent` or `self-review` based on the actual execution mode.
- **FR-041**: Analysis MUST persist finding identifiers across runs and MUST NOT recycle an identifier.
- **FR-042**: Findings MUST support at least `open`, `resolved`, `accepted`, and `deferred` states.
- **FR-043**: Each analysis rerun MUST distinguish new, resolved, and unchanged findings.
- **FR-044**: CRITICAL and HIGH findings MUST be blocking; MEDIUM and LOW findings MUST be non-blocking but require an explicit `resolved`, `accepted`, or `deferred` disposition before analysis can pass.
- **FR-045**: A constitution violation MUST remain blocking and MUST NOT be accepted or deferred through analysis disposition.
- **FR-046**: Analysis results MUST be `failed` when any blocking finding is open or any non-blocking finding lacks a disposition, `pass-clean` when every finding is resolved, and `pass-with-accepted` when no blocking finding is open and at least one non-blocking finding is explicitly accepted or deferred.
- **FR-047**: Only `pass-clean` and `pass-with-accepted` results with current inputs MUST permit `tasked` to transition to `analyzed`.
- **FR-048**: The first failed final analysis MAY initiate one user-approved correction and regeneration cycle.
- **FR-049**: If the next complete analysis also fails, the workflow MUST perform no further automatic rollback, correction, or regeneration.
...
- **FR-051**: Continuing after the correction-cycle limit MUST require a new direct user decision.
- **FR-052**: Implementation MUST refuse an absent, failed, stale, or undeclared-review-mode analysis even when the spec frontmatter says `analyzed`.
...
- **Review mode**: Evidence that semantic analysis ran either in a fresh independent context or as self-review.

## Clarifications

<!-- Populated by `/maxi:clarify`. -->
...
- **SC-009**: Implementation rejects 100% of fixtures with absent, failed, stale, or undeclared-review-mode analysis despite an `analyzed` spec status.
- **SC-010**: `bash tests/run-all.sh` passes with deterministic coverage for the artifact graph and skill-contract changes.

## Assumptions

- Mutable Maxi artifacts are normally modified through their owning skills; silent out-of-band edits that do not bump `revision` are unsupported and cannot be detected by revision metadata alone.
...
- Independent reviewer dispatch is preferred when supported; runtimes without isolation may produce an explicitly labelled self-review.
- Non-blocking findings may be accepted or deferred only through an explicit decision that remains visible in the analysis registry.
