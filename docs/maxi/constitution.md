...
Every structural choice (stack, pattern, dependency, pipeline deviation) generates an ADR, proposed automatically by the pipeline and written only with explicit consent. Every new ADR records its creating spec through a direct `spec` link containing the full spec slug, or `spec: null` when it is standalone, and its `design_cycle`; existing ADRs are not migrated, and a missing historical cycle means `0`. That link enables the sole exception to append-only bodies: an agent-proposed active-spec amendment may update an accepted ADR whose `spec` matches the current active spec and has a matching `design_cycle`, including after a rollback from `done`, only after the full amended ADR and exact diff are shown and explicitly approved. A `done` rollback increments the spec cycle, so a prior conception, missing or null link, or spec at `done`, `parked`, or `cancelled` uses closed-spec supersession instead.
...
**Version**: 1.5.0 | **Created**: 2026-05-24 | **Updated**: 2026-08-23
