...
if [ -f "$CLARIFY_SKILL" ]; then
  ...
  assert_grep "$CLARIFY_SKILL" '[Ss]top.*`REVIEW_REQUIRED`.*no public phase after.*review handoff' "clarify stops public phases at review handoff"
  assert_grep "$CLARIFY_SKILL" 'automatically invoke.*internal `x-review`.*never.*user' "clarify invokes its internal review handoff"
...
if [ -f "$PLAN_SKILL" ]; then
  ...
  assert_grep "$PLAN_SKILL" 'automatically invoke.*internal `x-review`.*never.*user' "plan invokes its internal review handoff"
  ...
  assert_grep "$PLAN_SKILL" 'REVIEW_REQUIRED.*automatically invoke.*internal `x-review`.*never.*user' "plan correction invokes its internal review handoff"
...
if [ -f "$REVISE_SKILL" ]; then
  ...
  assert_grep "$REVISE_SKILL" '[Ss]top.*`REVIEW_REQUIRED`.*no public phase after.*review handoff' "revise stops public phases at review handoff"
  assert_grep "$REVISE_SKILL" 'automatically invoke.*internal `x-review`.*never.*user' "revise invokes its internal review handoff"
