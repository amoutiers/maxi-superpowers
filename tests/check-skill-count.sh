...
for doc in "${SYNC_DOCS[@]}"; do
  ...
  assert_grep "$doc" "Every projection's exact distributed bytes are SHA-256-bound by its ordinary SDD ledger" "$label documents the projection-byte trust anchor"
  assert_grep "$doc" 'Removing an anchored incomplete `TNNN` during structural correction fails before successor creation and leaves the active-projection pointer unchanged' "$label preserves anchored incomplete tasks"
  assert_grep "$doc" 'Complete ledger lines containing `Ruling:` are preserved byte-for-byte in lineage order and hash-bound by the terminal receipt' "$label preserves ordered hash-bound ruling evidence"
  assert_grep "$doc" 'Fix round:\*\* All findings addressed, no new Critical/Important breakage' "$label documents the exact upstream positive fix-round conclusion"
  assert_not_grep "$doc" 'Fix round:.*no out-of-scope observation' "$label rejects the former local fix-round suffix"
