...
for doc in "${SYNC_DOCS[@]}"; do
  ...
  assert_grep "$doc" 'Owner-only calls return after their own write' "$label separates owner and destination"
  assert_grep "$doc" 'at most two passes' "$label bounds design review"
  assert_grep "$doc" 'Public `/maxi:review` is report-only' "$label preserves direct review scope"
...
for doc in "${ADR_POLICY_DOCS[@]}"; do
  ...
  assert_grep "$doc" 'including after a rollback from `done`' "$label keeps active reopened specs amendable"
  assert_grep "$doc" 'matching `design_cycle`' "$label restricts amendment to the current design cycle"
