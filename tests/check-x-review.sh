...
if [ -f "$TEMPLATE" ]; then
  ...
  assert_grep "$TEMPLATE" 'exact current bytes' "review template byte verification"
...
if [ -f "$SKILL" ]; then
  ...
  assert_grep "$SKILL" '[Rr]ecompute.*SHA-256.*exact current bytes' "x-review recomputes current SHA-256"
  assert_grep "$SKILL" 'returned path, revision, and SHA-256.*exactly equal.*envelope.*freshly reread subject' "x-review binds return identity to envelope and current subject"
