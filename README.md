...
An agent-proposed active-spec amendment may revise an accepted ADR only when its direct `spec` link matches the creating spec and that spec remains active (`drafting` through `implementing`), after you see the full amended ADR and exact diff and explicitly answer yes. Closed, missing, or null links use closed-spec supersession: create a new ADR that supersedes the old one. `/maxi:analyze` includes a Pass G that cross-checks ADRs against the constitution and each other.
