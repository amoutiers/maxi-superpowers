...
Every `spec.md` has a YAML frontmatter `status:` field and a `design_cycle:` nonnegative integer. New specs start at `0`; `/maxi:revise` increments it only when rolling `done` back into active design.
