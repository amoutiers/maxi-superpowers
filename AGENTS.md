...
The 10-state FSM remains unchanged. The three fixed review boundaries are design review after the normal plan write, readiness review in `/maxi:analyze` before implementation, and the upstream SDD final implementation review. `/maxi:review` writes `reviews/design-review.md` for the exact current `spec.md` and `plan.md`; a missing or stale approval stops task extraction before any write. Corrections stop after their owner write and never start a review or successor phase. Re-review is an explicit `/maxi:review` request.
