...
description: Use when reviewing maxi-superpowers' own authored docs for factual drift — before committing pipeline/skill changes, after editing CLAUDE.md/README/architecture.md, or when asked to check whether docs are consistent, in sync, or contradict each other
---
...
# (CLAUDE.md overview, docs/architecture.md, README). Also verify the breakdown
...
# Compare this exact set to the bullet list in CLAUDE.md's "Fast tier" section.
...
# Confirm the SAME set (and sequence, where shown) in CLAUDE.md, skills/using-maxi/SKILL.md,
...
grep -hoE 'drafting|specified|clarified|planned|tasked|analyzed|implementing|done|parked|cancelled' \
  CLAUDE.md skills/using-maxi/SKILL.md docs/pipeline-flow.md docs/delegation-map.md | sort -u
...
- **Mandatory-Sync-4 coherence** — `docs/pipeline-flow.md` (diagram + FSM), `docs/delegation-map.md`, `skills/using-maxi/SKILL.md`, `CLAUDE.md` must tell the same story: every skill, status, and transition appears consistently across all four.
...
When a fix touches a Mandatory-Sync-4 file, remind the user the other three may need the same edit.
...
| Fixing one Mandatory-Sync-4 file, forgetting the other three | Drift across the four is the original bug this guards against. |
